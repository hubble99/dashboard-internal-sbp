...
// Helper: bangun WHERE clause dinamis untuk raw SQL penyaluran
function buildRawWhere(filters: DashboardFilters, alias = 'f') {
    ...
    if (filters.jenisLembagaPenyedia) {
        const v = Array.isArray(filters.jenisLembagaPenyedia) ? filters.jenisLembagaPenyedia : [filters.jenisLembagaPenyedia];
        conditions.push(`${alias}.jenis_lembaga_penyedia = ANY($${i}::text[])`);
        params.push(v); i++;
    }
...
function buildWhereClause(filters: DashboardFilters): Prisma.FactPenyaluranPanganWhereInput {
    const where: Prisma.FactPenyaluranPanganWhereInput = {};
    ...
    if (filters.jenisLembagaPenyedia) {
        where.jenisLembagaPenyedia = Array.isArray(filters.jenisLembagaPenyedia)
            ? { in: filters.jenisLembagaPenyedia }
            : filters.jenisLembagaPenyedia;
    }
    if (filters.produkDonasi) {
        // Filter by produk ID dalam JSONB
...
// ─── KPI ─────────────────────────────────────────────────────────
export async function getPenyaluranKPI(filters: DashboardFilters) {
    const where = buildWhereClause(filters);

    const [totalKg, totalPenerima, lembagaAktif, provinsiCount, kabupatenCount] = await Promise.all([
        prisma.factPenyaluranPangan.aggregate({ where, _sum: { jumlahKg: true } }),
        prisma.factPenyaluranPangan.aggregate({ where, _sum: { jumlahPenerimaManfaat: true } }),
        prisma.factPenyaluranPangan.findMany({ where, select: { lembagaId: true }, distinct: ['lembagaId'] }),
        prisma.factPenyaluranPangan.findMany({ where, select: { provinsiId: true }, distinct: ['provinsiId'] }),
        prisma.factPenyaluranPangan.findMany({ where, select: { kabupatenId: true }, distinct: ['kabupatenId'] }),
    ]);

    return {
        totalPenyaluran: totalKg._sum.jumlahKg || 0,
        totalPenerimaManfaat: totalPenerima._sum.jumlahPenerimaManfaat || 0,
...
// ─── Chart Per Bulan (Kg) ─────────────────────────────────────────
export async function getPenyaluranChartPerBulan(filters: DashboardFilters) {
    const where = buildWhereClause(filters);
    const data = await prisma.factPenyaluranPangan.groupBy({
        by: ['tahun', 'bulan'],
        where,
        _sum: { jumlahKg: true },
    ...
    return buildChartData(data, filters, 'jumlahKg');
}

// ─── Chart Per Bulan (Penerima) ───────────────────────────────────
export async function getPenyaluranChartPenerimaPerBulan(filters: DashboardFilters) {
    const where = buildWhereClause(filters);
    const data = await prisma.factPenyaluranPangan.groupBy({
        by: ['tahun', 'bulan'],
        where,
        _sum: { jumlahPenerimaManfaat: true },
        orderBy: [{ tahun: 'asc' }, { bulan: 'asc' }],
    });

    return buildChartData(data, filters, 'jumlahPenerima');
}

function buildChartData(data: any[], filters: DashboardFilters, field: 'jumlahKg' | 'jumlahPenerima') {
    const monthNames = ['Jan', 'Feb', 'Mar', 'Apr', 'Mei', 'Jun', 'Jul', 'Ags', 'Sep', 'Okt', 'Nov', 'Des'];
    const result: Array<{ tahun: number; bulan: number; label: string; jumlahKg?: number; jumlahPenerima?: number }> = [];

    const hasYearFilter = !!filters.tahun;
    const hasMonthFilter = !!filters.bulan;

    const getValue = (d: any) => {
        if (field === 'jumlahKg') return d._sum.jumlahKg || 0;
        return d._sum.jumlahPenerimaManfaat || 0;
    };
    const makePoint = (year: number, month: number, value: number) => {
        if (field === 'jumlahKg') return { tahun: year, bulan: month, label: `${monthNames[month - 1]} ${year}`, jumlahKg: value };
        return { tahun: year, bulan: month, label: `${monthNames[month - 1]} ${year}`, jumlahPenerima: value };
    };
    ...
    if (hasYearFilter && hasMonthFilter) {
        ...
        for (const year of years) {
            for (const month of months) {
                ...
                result.push(makePoint(year, month, matched ? getValue(matched) : 0));
            }
        }
        ...
        for (const year of availableYears) {
            for (const month of months) {
                const matched = data.find(d => d.tahun === year && d.bulan === month);
                if (matched) result.push(makePoint(year, month, getValue(matched)));
            }
        }
    } else {
        const now = new Date();
...
// ─── Top Lembaga (Kg) ─────────────────────────────────────────────
export async function getPenyaluranTopLembaga(filters: DashboardFilters, limit = 10) {
    const where = buildWhereClause(filters);
    const data = await prisma.factPenyaluranPangan.groupBy({
        by: ['lembagaNama'],
        where,
...
// ─── Top Provinsi (Kg) ────────────────────────────────────────────
export async function getPenyaluranTopProvinsi(filters: DashboardFilters, limit = 10) {
    const where = buildWhereClause(filters);
    const data = await prisma.factPenyaluranPangan.groupBy({
        by: ['provinsiNama'],
        where,
...
// ─── Top Lembaga (Penerima) ───────────────────────────────────────
export async function getPenyaluranTopLembagaPenerima(filters: DashboardFilters, limit = 10) {
    const where = buildWhereClause(filters);
    const data = await prisma.factPenyaluranPangan.groupBy({
        by: ['lembagaNama'],
        where,
        _sum: { jumlahPenerimaManfaat: true },
        orderBy: { _sum: { jumlahPenerimaManfaat: 'desc' } },
        take: limit,
    });
    return data.map(d => ({ lembagaNama: d.lembagaNama, jumlahPenerima: d._sum.jumlahPenerimaManfaat || 0 }));
}

// ─── Top Provinsi (Penerima) ──────────────────────────────────────
export async function getPenyaluranTopProvinsiPenerima(filters: DashboardFilters, limit = 10) {
    const where = buildWhereClause(filters);
    const data = await prisma.factPenyaluranPangan.groupBy({
        by: ['provinsiNama'],
        where,
    ...
    return data.map(d => ({ provinsiNama: d.provinsiNama, jumlahPenerima: d._sum.jumlahPenerimaManfaat || 0 }));
}

// ─── Pivot: Lembaga × Provinsi (Kg) ──────────────────────────────
export async function getPenyaluranPivotProvinsi(filters: DashboardFilters, pagination: PaginationParams) {
    const where = buildWhereClause(filters);
    const { page, pageSize } = pagination;

    const [data, totalGroups, grandTotalAgg] = await Promise.all([
        prisma.factPenyaluranPangan.groupBy({
        ...
        prisma.factPenyaluranPangan.groupBy({ by: ['lembagaNama', 'provinsiNama'], where }),
        prisma.factPenyaluranPangan.aggregate({ where, _sum: { jumlahKg: true } }),
    ]);
...
// ─── Pivot: Lembaga × Provinsi (Penerima) ────────────────────────
export async function getPenyaluranPivotPenerima(filters: DashboardFilters, pagination: PaginationParams) {
    const where = buildWhereClause(filters);
    const { page, pageSize } = pagination;
    ...
    return {
        rows: data.map(d => ({ lembaga: d.lembagaNama, provinsi: d.provinsiNama, jumlahPenerima: d._sum.jumlahPenerimaManfaat || 0 })),
        total: totalGroups.length,
        grandTotal: grandTotalAgg._sum.jumlahPenerimaManfaat || 0,
        page, pageSize,
    };
}

// ─── Pivot: Lembaga × Produk Donasi (via JSONB expansion) ────────────────────
export async function getPenyaluranPivotProduk(filters: DashboardFilters, pagination: PaginationParams) {
    const { page, pageSize } = pagination;
    const { sql: whereSQL, params, nextIdx } = buildRawWhere(filters);

    // Extra: jika filter produkDonasi aktif, hanya tampilkan produk terpilih di pivot
    let extraValFilter = '';
